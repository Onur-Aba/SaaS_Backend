...
import { Repository, LessThan, DataSource } from 'typeorm'; // <-- DataSource eklendi
...
import { MembershipEntity } from '../memberships/entities/membership.entity'; // <-- MembershipEntity eklendi
...
export class AuthService {
  ...
  constructor(
    ...
    private readonly dataSource: DataSource, // <-- DataSource eklendi (Membership sorguları için)
  ) {}
  ...
  async login(loginDto: LoginDto, userAgent: string, ipAddress: string) {
    ...
    const sessionResult = await this.createSession(user, userAgent, ipAddress);

    return {
      ...sessionResult,
      // Login yanıtına tenant listesini ekledik
      tenants: availableTenants,
    };
    // --- DEĞİŞİKLİK BİTİŞİ ---
  }

  // --- YENİ METHOD: SWITCH TENANT ---
  async switchTenant(userId: string, targetTenantId: string) {
    // Kullanıcı gerçekten bu tenant'ın üyesi mi?
    const membership = await this.dataSource.getRepository(MembershipEntity).findOne({
      where: { user_id: userId, tenant_id: targetTenantId },
    });
    ...
    // Refresh token genellikle global kalabilir ama strict modda o da tenant'lı olabilir.
    // Basitlik için sadece Access Token'ı özelleştiriyoruz.
  ...
  async verify2Fa(verify2FaDto: Verify2FaDto, ip: string, userAgent: string) {
    ...
    try {
      ...
      // NOT: 2FA sonrası da tenant listesi dönmek isterseniz login'deki mantığı buraya da taşıyabilirsiniz.
      // Şimdilik sadece session açıyoruz.
